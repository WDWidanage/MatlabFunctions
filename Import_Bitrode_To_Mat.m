function Import_Bitrode_To_Mat(varargin)
% Syntax of use
...
for n = 1:nChannels
    if iscell(data{n})
        ...
        % If the cell string consisits of a ':' then this will be a time strin column and therefore porcess the data 
...
if isempty(channelString)           % No headers are specified. Use default, Channel_1...Channel_N
    ...
    % Handle units
    expHdrsWithUnits = {'Total Time','Step time','Current','Voltage','Power', 'Amp-Hours','Watt-Hours','Amp Hours Charge','Amp Hours Discharge','Watt Hours Charge','Watt Hours Discharge','Temperature A1'};
    units = repmat({'None'},nChannels,1);
    channelText_Units = regexp(channelString,'[\w\s-#]{2,}','match'); % Get channel hdr names
    for ii = 1: length(expHdrsWithUnits)
        [~,idxU] = find(ismember(channelText_Units,expHdrsWithUnits{ii})); % Idx relative to units hdr and units strings
        [~,idx] = find(ismember(channelNames,expHdrsWithUnits{ii}));       % Idx relative to hdr strings
        
        if ii == 1|| ii== 2 % Handle Total Time and Step time units
            if isnan(dataConv(idx))
                units(idx) = {'s'};
            else
                units(idx) = channelText_Units(idxU+1);
            end
        elseif ii == 8 % Handle for temperature
            units(idx) = {'degC'};
        else
            units(idx) = channelText_Units(idxU+1);
        end
