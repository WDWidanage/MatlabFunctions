function [du,iterUpdate,iterPCG,flag] = Derivative(u,x,alpha,iterMax,iterPCGMax,dia)
%
...
while norm(deltaM) > 1E-4 && iterUpdate <= iterMax
    ...
    if dia
        if numel(dx)==1
            ...
            relTime = cumsum(dx);
            relTime = relTime -relTime(1);             % Relative horizontal vector now starts at zero
        end
